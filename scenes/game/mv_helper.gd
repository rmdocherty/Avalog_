...
func get_square_mvs(inner_mag: float, outer_mag: float, angle_from: float, angle_to: float, 
					mv_type: int=cst.mv_type.NORMAL, delta_inner: bool=false, n_incs: int=32) -> MoveVector:
	if delta_inner:
		inner_mag = outer_mag - 0.2 * cst.LOGIC_PIECE_RADIUS / cst.LOGIC_SQ_W
	var from_circ = PackedVector2Array([])
	var to_circ = PackedVector2Array([])
	var dir_circ = PackedVector2Array([])
	...
	for i in range(resolution):
		var angle_rad = 2 * PI * (i / float(n_incs))
		var direction = Vector2(cos(offset_rad + angle_rad), sin(offset_rad + angle_rad))
		if 2 * PI * (315. / 360) <= angle_rad or angle_rad <= 2 * PI * (45. / 360):
			direction[0] = 1
		if 2 * PI * (45. / 360) <= angle_rad and angle_rad <= 2 * PI * (135. / 360):
			direction[1] = 1
		if 2 * PI * (135. / 360) <= angle_rad and angle_rad <= 2 * PI * (225. / 360):
			direction[0] = -1
		if 2 * PI * (225. / 360) <= angle_rad and angle_rad <= 2 * PI * (315. / 360):
			direction[1] = -1
		...
		dir_circ.append((outer_point - inner_point))
	var move_vector: MoveVector = MoveVector.new()
	move_vector.set_all(cst.draw_type.RADIAL, mv_type, to_circ, from_circ, dir_circ)
	return move_vector
